...
def parse_options():
    ...
    ap.add_argument("root", help="Directory to build an image from; Must contain a Dockerfile.", type=Path)
    ap.add_argument("-d", "--dryrun", help="Print commands instead of running them.", action="store_true")
    ap.add_argument("-x", "--docker-executable", help="Path to the docker executable.", type=str, default="docker")

    opts = ap.parse_args()
    return opts


def get_docker_image_id_by_name(name: str, docker_executable: str = "docker") -> str:
    output = run(
        f'{docker_executable} image ls --format="{{.Repository}} {{.ID}}"',
        shell=True,
        stderr=DEVNULL,
        stdout=PIPE,
    )
...
def check_binary_exists(bin_name: str) -> bool:
    return call(f"which {bin_name}", shell=True, stdout=DEVNULL, stderr=DEVNULL) < 1
...
def main():
    ...
    if repository == "beaker":
        *workspace, name = name.split("/")
        ...
        if "," in architectures:
            msg = "Cannot provided multiple architectures when " "uploading to Beaker."
    ...
    try:
        if repository == "docker":
            ...
            # remove old image
            run_commands(
                f"{opts.docker_executable} buildx rm {name}:{version}",
                f"{opts.docker_executable} image rm {name}:{version}",
                dryrun=opts.dryrun,
                stop_on_error=False,
            )
            ...
            if not opts.dryrun:
                image_id = get_docker_image_id_by_name(name, docker_executable=opts.docker_executable)

            run_commands(f"{opts.docker_executable} tag {image_id} {workspace}", dryrun=opts.dryrun)
            run_commands(f"{opts.docker_executable} push {workspace}", dryrun=opts.dryrun)
        ...
        # REMOVE BUILDER
        cmd = f"{opts.docker_executable} buildx rm {builder_name}"
        if opts.dryrun:
            print(f"DRYRUN: {cmd}")
        else:
            out = call(cmd, shell=True)
